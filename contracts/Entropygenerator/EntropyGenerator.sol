...
contract EntropyGenerator is Ownable {
    ...
    // Using the block number and a constant seed to determine the index
    // where 999999 will be "naturally" placed. This keeps it unpredictable
    // but deterministic within this transaction/block.
    uint256 seed = uint256(keccak256(abi.encodePacked(block.number)));
    uint256 indexFor999999 = 512 + (seed % 256); // Ensuring it falls within [512, 768)
    ...
    unchecked {
        for (uint256 i = lastInitializedIndex; i < 770; i++) {
            ...
            if (i == indexFor999999) {
                // Deterministically ensure 999999 appears in the desired range
                pseudoRandomValue = 999999;
            } else {
                // Regular pseudo-random value generation
    ...
    lastInitializedIndex = 770; // Mark batch as complete
}
...
function getEntropy(
    ...
    uint256 slotValue = entropySlots[slotIndex]; // get the entropy value for the slot
    uint256 entropy = (slotValue / (10 ** (72 - position))) % 1000000; // extract the relevant digits to form the entropy value
